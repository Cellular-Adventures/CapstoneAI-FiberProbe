...
def plot_dist_labeled(df, valid_test_results_10, path_to_output):
    """Plot and save histogram comparing true and predicted valid bubble speeds.

    Args:
        df (pd.DataFrame): DataFrame containing the true velocities in column "VeloOut".
        valid_test_results_10 (pd.DataFrame): DataFrame with predictions (column "final prediction").
        path_to_output (str): Directory path to save the output image.
    """
    ...
    # Create grouped bars
    plt.bar(bin_centers - width/2, true_counts, width=width,
            label="True Speeds", edgecolor='black')
    plt.bar(bin_centers + width/2, pred_counts, width=width,
            label="Predicted Speeds", edgecolor='black')
...
def plot_dist_all(df, outcome_df, path_to_output):
    """Plot and save histogram comparing all true and ensemble-predicted bubble speeds.

    Args:
        df (pd.DataFrame): DataFrame containing the true velocities in column "VeloOut".
        outcome_df (pd.DataFrame): DataFrame with predictions
            (column "final prediction" and "Standard deviation %").
        path_to_output (str): Directory path to save the output image.
    """
    valid_df = df[df["VeloOut"] != -1]
...
def plot_parity_all(y_velo, outcome_df_valid, path_to_output):
    """Plot and save a parity plot for all model predictions vs. true labels.

    Args:
        y_velo (np.ndarray): Array of true velocity values.
        outcome_df_valid (pd.DataFrame): DataFrame with predictions for valid data.
        path_to_output (str): Directory path to save the output image.
    """
    ...
    # ±10% error bounds
    plt.fill_between(x_vals, x_vals * 0.9, x_vals * 1.1, color='gray',
                     alpha=0.2, label='±10% error margin')

    # Scatter points
    plt.scatter(y_velo, outcome_df_valid['predictions model 1'], color='blue',
                alpha=0.2, s=10, label='Model 1')
    plt.scatter(y_velo, outcome_df_valid['predictions model 2'], color='green',
                alpha=0.2, s=10, label='Model 2')
    plt.scatter(y_velo, outcome_df_valid['predictions model 3'], color='red',
                alpha=0.2, s=10, label='Model 3')
    plt.scatter(y_velo, outcome_df_valid['predictions model 4'], color='purple',
                alpha=0.2, s=10, label='Model 4')
    plt.scatter(y_velo, outcome_df_valid['predictions model 5'], color='orange',
                alpha=0.2, s=10, label='Model 5')
    plt.scatter(y_velo, outcome_df_valid['final prediction'], color='black',
                alpha=0.7, s=15, label='Unfiltered Ensemble')
...
def plot_parity_separate(y_velo, outcome_df_valid, path_to_output):
    """Plot and save separate parity plots for each model and the ensemble.
...
def plot_parity_ensemble(y_velo, outcome_df_valid, path_to_output):
    """Plot and save a parity plot for accepted ensemble predictions.
    ...
    # Scatter points
    plt.scatter(accepted_df_valid['True'], accepted_df_valid['final prediction'],
                color='black', alpha=0.5, s=15, label='Ensemble')
...
def plot_check_accepted(outcome_df, df_check, df, path_to_output):
    """Plot and save random examples of accepted unlabeled bubbles with low uncertainty.

    Args:
        outcome_df (pd.DataFrame): DataFrame with model predictions and uncertainty.
        df_check (pd.DataFrame): DataFrame with voltage signals ("VoltageOut").
        df (pd.DataFrame): DataFrame with true velocities ("VeloOut").
        path_to_output (str): Directory path to save the output image.
    """
    ...
    random_indices = np.random.choice(
        len(unlabeled_df_accepted),
        size=min(n_samples, len(unlabeled_df_accepted)),
        replace=False
    )
...
def plot_check_rejected(outcome_df, df_check, df, path_to_output):
    """Plot and save random examples of rejected unlabeled bubbles with high uncertainty.
